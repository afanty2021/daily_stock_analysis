...
const PortfolioPage: React.FC = () => {
  ...
  // Update WebSocket subscription when positions change
  useEffect(() => {
  ...
  // Refresh connection status indicator after snapshot reloads
  useEffect(() => {
    if (wsClientRef.current) {
                  ...
                  {positionRows.map((row) => {
                    ...
                    <tr key={rowKey} className="border-b border-white/5">
                      ...
                      <td className="py-2 pr-2 text-right">
                        <div className={rowWithPrice.isRealtime ? 'text-cyan-400' : ''}>{formatPositionPrice(rowWithPrice)}</div>
                        <div className={`text-[11px] ${hasPositionPrice(rowWithPrice) ? 'text-secondary' : 'text-warning'}`}>
                          {rowWithPrice.isRealtime && <span className="ml-1 text-xs text-cyan-400">●</span>}
                          {getPositionPriceLabel(rowWithPrice)}
                        </div>
                      </td>
